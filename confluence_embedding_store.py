...
# Separate FAISS-backed embedding store for Confluence chat, persisted under ./data/confluence
DATA_DIR = os.path.join(os.getcwd(), "data", "confluence")
...
def init(client, embedding_model_name: str = "text-embedding-3-small"):
    """Bind OpenAI client and load or create FAISS index for Confluence chat."""
...
def _embed_texts(texts: List[str]) -> np.ndarray:
    ...
    if _CLIENT is None or _EMBED_MODEL is None:
        raise RuntimeError("confluence_embedding_store not initialized. Call init(client, embedding_model_name) first.")
...
def clear_all() -> Dict[str, Any]:
    """Delete the Confluence FAISS index and metadata, and reset state."""
