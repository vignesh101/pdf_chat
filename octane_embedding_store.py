...
# Separate FAISS-backed embedding store for Octane chat, persisted under ./data/octane
DATA_DIR = os.path.join(os.getcwd(), "data", "octane")
...
def init(client, embedding_model_name: str = "text-embedding-3-small"):
    """Bind OpenAI client and load or create FAISS index for Octane chat."""
    ...
    _META.setdefault("files", [])  # list of {file_id(key), name(title), created_at}
...
def _embed_texts(texts: List[str]) -> np.ndarray:
    ...
    if _CLIENT is None or _EMBED_MODEL is None:
        raise RuntimeError("octane_embedding_store not initialized. Call init(client, embedding_model_name) first.")
...
def add_item(key: str, title: Optional[str], text: str) -> str:
    """Add an Octane entity's text into the Octane FAISS store.

    Uses key as file_id and title (or key) as name for metadata; returns the key.
    """
    global _META, _INDEX, _DIM
    if not (text or "").strip():
        return key or "octane:empty"
    ...
    # Register file metadata entry (deduplicate by key)
    try:
    ...
    try:
        ...
        exists = any((f.get("file_id") == key) for f in _META["files"])
        if not exists:
            _META["files"].append({"file_id": key, "name": (title or key or ""), "created_at": created_at})
    except Exception:
        pass
...
def clear_all() -> Dict[str, Any]:
    """Delete the Octane FAISS index and metadata, and reset state."""
